function create_piechart(dataset, viewerWidth, viewerHeight, div, div_tooltip, div_legend, input_name) {
	...
	g.append("path")
        ...
        .on("mousemove",mousemove_piechart)
		.on("click", function(d) {
			$(input_name).val(d.data.legend);
			new_search();
		});
    ...
    function mousemove_piechart(d){
        var div = d3.select("#piechart_tooltip");
        var mouseVal = d3.mouse(this);
        div.style("display","none");
        div.html(d.data.legend.replace("_", " ").replace("_", " ").replace("_", " "))
            .style("left", (mouseVal[0]+60) + "px")
            .style("top", (mouseVal[1]+130) + "px")
            .style("opacity", 1)
            .style("display","block");
    };
	drawLegend();
