function create_sunburst(width, height, div_container,div_tooltip,div_legend,data,colors,levels,total_objects_sunburst){
	// Use d3.text and d3.csv.parseRows so that we do not need to have a header
	// row, and can receive the csv as an array of arrays.
	var familySunburst = new Sunburst(width, height, div_container,div_tooltip,div_legend,data,colors,levels,total_objects_sunburst);
	familySunburst.buildHierarchy();
	familySunburst.createVisualization();
}

function Sunburst(width, height, div_container,div_tooltip,div_legend,data,colors,levels,total_objects_sunburst){
	this.data=data;
	this.colors=colors;
	this.levels=levels;
	...
	function mousemove_sunburst(d){
		...
		div.style("display","none");
		div.html(d.name.split(".").pop()+"</br>"+percentageString)
    ...
    if(!d.name){d.name=d.key;}
		...
		var get_arg = "";
		for (var j = 0; j < parts.length; j++) {
			$("#"+levels[j]).val(parts[j].replace(/:/g, "."));
		}
		$( "#search" ).submit();
	};
